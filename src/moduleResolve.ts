...
import {
  ...
  normalizeSlashes,
  trimExtension
} from 'typescript-to-lua/dist/utils';
import {
  couldNotReadDependency,
  couldNotResolveRequire
} from 'typescript-to-lua/dist/transpilation/diagnostics';
import {
  BuildMode,
  CompilerOptions
...
class ResolutionContext {
  ...
  public resolveImport(file: ProcessedFile, required: LuaRequire): void {
    ...
    if (this.options.tstlVerbose) {
      console.log(
        `Resolved ${required.requirePath} to ${normalizeSlashes(
          dependencyPath
        )}`
      );
    }
  ...
  private couldNotResolveImport(
    ...
    this.diagnostics.push(
      couldNotResolveRequire(
        ...
        path.relative(getProjectRoot(this.program), file.fileName)
      )
    );
  }
  ...
  private resolveDependencyPath(
    ...
    if (this.options.tstlVerbose) {
      console.log(
        `Resolving "${dependency}" from ${normalizeSlashes(
          requiringFile.fileName
        )}`
      );
    }
    ...
    try {
      const resolveResult = resolver.resolveSync(
        {},
        fileDirectory,
        dependencyPath
      );
  ...
  private tryGetModuleNameFromPaths(
    ...
    const relativeImport = removeTrailingDirectorySeparator(
      normalizeSlashes(relativeToBaseUrl)
    );
      ...
      if (indexOfStar !== -1) {
        ...
        if (
          ...
          for (const target of targetPatterns) {
            const file = this.getFileFromPath(
              path.join(baseUrl, target.replace('*', matchedStar))
            );
            if (file) return file;
          }
        }
...
export function resolveDependencies(
  ...
  for (const file of files) {
    if (options.tstlVerbose) {
      console.log(
        `Resolving dependencies for ${normalizeSlashes(file.fileName)}`
      );
    }
...
function shouldRewriteRequires(
...
function replaceInSourceMap(
  ...
  if (
    ...
    parent.children = [
      new SourceNode(node.line, node.column, node.source, [resolvedRequire])
    ];
...
function fallbackResolve(
  ...
  return formatPathToLuaPath(
    path
      .normalize(
        path.join(path.relative(sourceRootDir, fileDir), required.requirePath)
      )
      .split(path.sep)
      .filter((s) => s !== '.' && s !== '..')
      .join(path.sep)
