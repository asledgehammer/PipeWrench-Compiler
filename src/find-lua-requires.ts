...
type MatchResult<T> =
  | { matched: true; match: T }
  | { matched: false; end: number };
...
function matchRequire(lua: string, offset: number): MatchResult<LuaRequire> {
  ...
  const { value: requireString, offset: offsetAfterString } = readString(
    lua,
    offset,
    lua[offset]
  );
  ...
  return {
    matched: true,
    match: { from: start, to: offset - 1, requirePath: requireString }
  };
}
...
function expect(lua: string, offset: number, char: string) {
  if (lua[offset] !== char) {
    throw new Error(
      `Expected ${char} at position ${offset} but found ${lua[offset]}`
    );
  }
}
